...
class MemberTaskCompletedPage extends StatefulWidget {
  const MemberTaskCompletedPage({super.key, required this.title, required this.isJoinServer});
...
class _MemberTaskCompletedPageState extends State<MemberTaskCompletedPage> {
  ...
  Widget build(BuildContext context) {
    BlocProvider.of<TaskListCubit>(context).getTaskCompletedMember();
    ...
    return BlocBuilder<TaskListCubit, DataState>(
        builder: (context, state) {
          ...
          if (tasks.isEmpty) {
            return EmError(
              onPressed: () {},
              textAbove: 'Anda belum mempunyai tugas selesai.',
              textBelow: 'Selesaikan tugas aktif terlebih dahulu!',
            );
          }
